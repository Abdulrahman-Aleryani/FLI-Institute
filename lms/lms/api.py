...
_attendance_spec = importlib.util.spec_from_file_location(
	"lms.lms.api.attendance",
	str(Path(__file__).resolve().parents[1] / "api" / "attendance.py"),
)
_attendance_module = importlib.util.module_from_spec(_attendance_spec)
_attendance_spec.loader.exec_module(_attendance_module)
sys.modules["lms.lms.api.attendance"] = _attendance_module
...
def get_user_info():
	...
	user.is_interviewer = "Interviewer" in user.roles
	user.is_student = (
		not user.is_instructor
		and not user.is_moderator
		and not user.is_evaluator
		and not user.is_interviewer
	)
...
@frappe.whitelist()
def get_enrolled_students(batch_name=None):
	if not batch_name:
		return []
...
@frappe.whitelist()
def get_or_create_grade_sheet(batch_name=None):
	if not batch_name:
	...
	if existing:
		...
		_cleanup_invalid_user_permission_doctypes()
...
def _cleanup_invalid_user_permission_doctypes():
	if getattr(frappe.flags, "lms_cleaned_invalid_user_permissions", False):
		return

	allows = frappe.get_all("User Permission", pluck="allow", distinct=True)
	missing_allows = [allow for allow in allows if allow and not frappe.db.exists("DocType", allow)]

	if missing_allows:
		frappe.db.delete("User Permission", {"allow": ["in", missing_allows]})

	frappe.flags.lms_cleaned_invalid_user_permissions = True
...
def get_homepage_courses():
	"""Return limited course info for the public homepage."""
	return frappe.get_all(
		"LMS Course",
		{
			"published": 1,
